...
async def linking_requests_queue(request: web.Request) -> web.Response:
    a = await auth_required(request, permissions=5)
    if a.status != 200:
        return a
    db = request.config_dict["DB"]
...
async def edit_user(request: web.Request) -> web.Response:
    ...
    if a.user_id == user_id:
        pass
    elif (await auth_required(request, permissions=5)).status == 200:
...
async def get_meets_within_two_weeks(request: web.Request) -> web.Response:
    ...
    dateweekbefore = datenow - datetime.timedelta(days=3)
