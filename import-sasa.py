...
def assemble_event_code(e, split, gender = None):
    code = ""
    if gender:
        code += gender
    else:
...
if "--import" in sys.argv:
    ...
    with open("creds-sasa.json", "r") as f:
        creds = json.load(f)
            ...
            try:
                if entry["swimmers"][0]["team_code"] == TEAM:
                    if entry["relay"]:
                        ...
                        lead_event_code = assemble_event_code(ev, True, gender_id(lead_swimmer["gender"]))
                        if lead_event_code[-1] == "M":
                            lead_event_code = assemble_event_code(ev, True, gender_id(lead_swimmer["gender"]))[:-1] + "B"
    ...
    for result in m:
        c = cur.execute(
            f"SELECT id, dob, usas_id FROM swimmers WHERE usas_id = '{result['usa_swimming_id']}'"
        )
        r = cur.fetchone()
        if r:
            id = str(r[0])
            ...
            try:
                ...
                res = input(f"create new swimmer {result['name']}? ")
                if res.lower() == "y":
                    id = generate_id(1)
                    print(id)
                    c = cur.execute(f"INSERT INTO swimmers (id, first_name, last_name, middle_name, team, gender, dob, usas_id) VALUES ({id}, '{f_name}', '{l_name}', '{m_name}', 'SASA', '{gender_id(result['gender'])}', '{result['dob']}', '{result['usa_swimming_id']}')")
                else:
                    print("not creating new swimmer")
                    continue
        if result['usa_swimming_id'] == usas_id and result['dob'] != dob and result['dob'] != "0000-00-00":
            res = input(f"Would you like to update {result['name']} DOB from {dob} to {result['dob']}? ")
            if res.lower() == "y":
                cur.execute(f"UPDATE swimmers SET dob = '{result['dob']}' WHERE id = {id}")
