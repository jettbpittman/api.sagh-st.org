...
for result in m:
    ...
    c = con.execute(f"SELECT id FROM swimmers WHERE usas_id = '{result['usa_swimming_id']}'")
    r = c.fetchone()
    c.close()
    if r:
        id = str(r[0])
        ...
        l_name = name[0].strip()
        f_name = name[1].strip()
        c = con.execute(f"SELECT id FROM swimmers WHERE last_name = '{l_name}' AND first_name = '{f_name}'")
        r = c.fetchone()
        try:
            id = str(r[0])
        except TypeError:
    ...
    con.execute(
        ...
        [generate_id(3), id, MEET, result['event'], result['seed'], result['time'], splits]
    )
    con.commit()
    time.sleep(1)
